  ...
  try {
    ...
    try {
      if (llm.isConfigured()) {
        ...
        const realityRule = `【非常重要】每个分镜是由一张静态图生成的约3-5秒短片，只能做缓慢推近/轻移/轻微旋转等"轻运镜"，演不出"拧开盖子/倒水/翻转/手部操作"等复杂动作。所以：(a) visual 要拍"一个有说服力的状态/瞬间"(例：盖子已拧开摆在旁、露出内胆与厚密封圈；产品细节微距；模特手持商品微笑)，不要写动作过程；(b) 文案只描述画面看得到的状态/卖点，绝不承诺画面演不出的动作——例如别写"拧开超顺滑"(演不出拧)，改成"密封圈厚实、倒提都不漏"这种描述状态/结果的说法。(c)【禁止凭空叠加产品图】绝不要写"画面上方/角落叠加产品图、产品图悬浮在画面、产品缩略图浮现"这类——AI 出图无法像后期那样干净叠一张图，只会把产品画成漂浮、变形的怪物悬在头顶；产品只能自然存在于场景中(被佩戴/手持/置于真实台面)。`;
