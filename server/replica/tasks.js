...
export function runTask(taskId, runner) {
  ...
  (async () => {
    try {
      ...
      try { await updateTask(taskId, { status: 'failed', error_message: String(e?.message || e), output_json: { failed: true, code: e?.code || null, notes: Array.isArray(e?.notes) ? e.notes : [] } }); }
